...
import { createImageUrl } from '../src/services/movieServices';
...
<img className="w-full h-40 block object-cover object-top"
 ...
 <div className="absolute top-0 left-0 w-full h-40 bg-black/80 opacity-0 hover:opacity-100">
  ...
  <p onClick= {markFavShow} className="cursor-pointer">  { <BiCalendarHeart  size={20} className="absolute top-2 left-2 text-pink-400"/>} </p>
  {/* <p>{like ? <FaHeart  size={20} className="absolute top-2 left-2 text-pink-500"/>:<FaRegHeart size={20} className="absolute top-2 left-2 text-gray-300"/>} </p> */}
 </div>
    </div>
      ); 
